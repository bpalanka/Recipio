    ...
    if recipe_results:
        ...
        for recipe in recipe_results:
            ...
            if 'name' in recipe and 'ingredients' in recipe:
                print("excluded list", excl)
                for excl in ingredExcl:
                    #if excl.lower() in recipe['name'].lower() or any(excl.lower() in ing.lower() for ing in recipe['ingredients']):
                    print(excl.lower(), " ", (recipe['name'].lower()))
                    if excl.lower()._contains_(recipe['name'].lower()): #or any(excl.lower() in ing.lower() for ing in recipe['ingredients']):
                        print("**", recipe['name'])
                        excluded = True
                        break
            elif not excluded:
